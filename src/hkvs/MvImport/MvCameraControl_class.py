...
def load_sdk_dll(dll_name):
    """智能加载SDK目录下的DLL"""
    if hasattr(sys, '_MEIPASS'):
        base_path = sys._MEIPASS  # 打包环境
    else:
        # 开发环境：当前文件 → 上级目录 → 同级目录的src
        current_file_path = Path(__file__).resolve()
        base_path = current_file_path.parent.parent.parent.parent  # 根据实际结构调整层级数
    # 构建完整DLL路径
    # dll_path = base_path / "src" / "hkvs" / "sdk" / "Runtime" / "Win64_x64" / dll_name
    dll_path = os.path.join(base_path, "src", "hkvs", "sdk", "Runtime", "Win64_x64", dll_name)
    print(dll_path)
    
    return CDLL(str(dll_path), winmode=0)

# MvCamCtrldll = WinDLL("MvCameraControl.dll")
# MvCamCtrldll = CDLL("MvCameraControl", winmode=0)
# MvCamCtrldll = CDLL("src\hkvs\sdk\Runtime\Win64_x64\MvCameraControl.dll", winmode=0)
MvCamCtrldll = load_sdk_dll("MvCameraControl.dll")
MvPolarizerdll = load_sdk_dll("MvPolarizer.dll")
# MvPolarizerdll=CDLL("MvPolarizer", winmode=0)
...
class MvCamera():
    ...
    @staticmethod
    def MV_CC_Image2Polarizer(pSrcImage, nSrcImageWidth, nSrcImageHeight, 
                              pOutImage1, pOutImage2, pOutImage3, pOutImage4, 
                              nThrDop):
        """
        调用 C 语言的 MV_CC_Image2Polarizer 函数，实现偏振图像转换
        :param pSrcImage: 输入原始图像数据（mono8）
        :param nSrcImageWidth: 输入图像宽度
        :param nSrcImageHeight: 输入图像高度
        :param pOutImage1: 偏振拆分灰度图
        :param pOutImage2: 去偏振灰度图
        :param pOutImage3: 偏振度图
        :param pOutImage4: 偏振伪彩图
        :param nThrDop: 低信号阈值，默认5
        :return: 成功返回 MV_OK，失败返回 MV_E_UNKNOW
        """

        # 设置参数类型
        MvPolarizerdll.MV_CC_Image2Polarizer.argtypes = (
            POINTER(c_ubyte),  # pSrcImage
            c_uint,            # nSrcImageWidth
            c_uint,            # nSrcImageHeight
            POINTER(c_ubyte),  # pOutImage1
            POINTER(c_ubyte),  # pOutImage2
            POINTER(c_ubyte),  # pOutImage3
            POINTER(c_ubyte),  # pOutImage4
            c_int              # nThrDop
        )

        # 设置返回类型
        MvPolarizerdll.MV_CC_Image2Polarizer.restype = c_int

        # 调用 C 函数
        return MvPolarizerdll.MV_CC_Image2Polarizer(
            pSrcImage, nSrcImageWidth, nSrcImageHeight, 
            pOutImage1, pOutImage2, pOutImage3, pOutImage4, 
            nThrDop
        )
